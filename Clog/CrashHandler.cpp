...
static LONG WINAPI UnhandledExceptionHandler(EXCEPTION_POINTERS* pExceptionInfo)
{
...

	{
		...
		oss << "ExceptionCode: 0x" << std::hex << code;
		LOG_FATAL(oss.str());
	}

	{
		std::ostringstream oss;
		oss << "ExceptionAddress: 0x" << addr;
		LOG_FATAL(oss.str());
	}
	...
	if (pExceptionInfo->ExceptionRecord->NumberParameters > 0)
	{
		...
		{
			std::ostringstream oss;
			oss << "ExceptionParameter[" << i << "]: 0x"
				<< std::hex << pExceptionInfo->ExceptionRecord->ExceptionInformation[i];
			LOG_FATAL(oss.str());
		}
	}
